...
export const getPostEventFields = (eventId: string, setFormFields: Dispatch<React.SetStateAction<FormFieldType[]>>, setData: Dispatch<React.SetStateAction<Data>>) => {
    publicGateway
        .get(makeMyPass.getPostEventFields(eventId))
        .then((response) => {
            setFormFields(response.data.response.questions);
            setData({ title: response.data.response.title, description: response.data.response.description });
        })
        .catch((error) => {
